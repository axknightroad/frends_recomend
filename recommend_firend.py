...
class User(object):
    ...
    def __init__(self,uid,net):
        ...
        self.network=net.get_self_network(net.list_map)
        self.circles=[]    #用户的朋友圈
        self.friend_max_circle={}  #潜在朋友的最大可能性圈子
        self.node_similarity_list=[0 for i in range(65535)]  #用户与周围朋友相似性list
            ...
            if ei not in searched_edges:
                ...
                for ej in edge_circle:
                    for ek in self.network[self.uid]:
                        if ek not in searched_edges and self.net.edge_similarity_dict[self.uid][(ej,ek)]>sv:#ei,ek??
    ...
    def get_candidate(self):
        candidate=[]
        for s in self.net.list_map[self.uid]:
            for e in self.net.list_map[s]:
                if e not in self.net.list_map[self.uid] and e not in candidate and e!=self.uid:
                    candidate.append(e)
    ...
    def get_similarity(self,target):
        ...
        self.node_similarity_list[target.uid]=similarity            
        return similarity

    #推荐好友
    def recommend(self,k,sv,ncc):
        candidate=self.get_candidate()
    #print "candidate size = %d"%len(candidate)
        temp_num=0
        for node in candidate:
            now_net=UserNetwork(node,self.net.list_map)
            #print "No.%d"%temp_num,"step 1"
            now_node=User(node,now_net)
            #print "No.%d"%temp_num,"step 2"
            now_node.get_circle(sv,ncc)
            #print "No.%d"%temp_num,"step 3"
            self.get_similarity(now_node)
            #print "No.%d"%temp_num,"step 4"
            temp_num+=1
        protential_firends=[]
        for i in range(k):
            uid=self.node_similarity_list.index(max(self.node_similarity_list))
            protential_firends.append(uid)
            self.node_similarity_list[uid]=0
...
file_name='facebook-links.txt.anon'#raw_input("> ")
#print "input node number in the map please"
node_num=65535#raw_input("> ")
print "input user please"#4 227
uid=int(raw_input("> "))
...
for circle in user.circles:
    print "this is user %d's circle %d:"%(user.uid,circle_num),circle
    circle_num+=1
user.get_candidate()
#print len(candidata)
